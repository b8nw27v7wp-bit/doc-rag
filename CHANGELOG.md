...
- 新增零依赖本地解析格式：HTML（去标签/脚本、块级转行、实体解码）与 CSV / TSV（支持引号包裹与转义，转制表符文本）
- 性能：多查询检索时 BM25 索引只构建一次并跨查询复用（`buildBM25Index` + `hybridSearch` 可选 `bm25` 参数）
- 架构优化：抽取 `lib/llm-config.ts` 集中解析 BYOK/环境变量 LLM 配置（chat 与 summarize 复用），`ssrf.ts` 新增 `isLocalBaseURL`
- 单测新增 10 项（HTML/CSV/TSV 解析、BM25 索引复用一致性、本地端点判定），共 124 项全绿

### 已知限制

- PDF 仅文本层解析；扫描件需先 OCR
- 检索为全量暴力计算（向量余弦 + BM25），超大库（万级块）需引入 ANN
- HTML 解析为启发式（正则去标签），复杂富文本可能残留噪声

## [0.6.0] - 2026-08-19

### Added

- 文档标签：入库时按 BM25 同款分词词频自动提取关键词（零 LLM 成本），文档页展示
- 文档摘要：可选，调用 LLM 生成一句话摘要并回填存储（`/api/documents/summarize`），文档页「摘要」按钮触发
- 会话管理增强：置顶（pinned，置顶会话排最前）+ 侧栏标题搜索
- 检索性能：`allChunks()` 内存缓存（BLOB→Float32 只反序列化一次），文档增删自动失效
- 数据层迁移：documents 新增 `keywords`/`summary` 列，sessions 新增 `pinned` 列（幂等 ALTER）
- 单测新增 11 项（关键词提取/摘要 prompt/置顶排序/缓存失效/关键词摘要存取），共 114 项全绿

### 已知限制

- PDF 仅文本层解析；扫描件需先 OCR
- 检索为全量暴力计算（向量余弦 + BM25），超大库（万级块）需引入 ANN
- 摘要与查询增强依赖 LLM 可用，离线/未配 Key 时自动回退
